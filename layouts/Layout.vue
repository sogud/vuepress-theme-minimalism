<template>
  <div class="theme-container" :class="isDark ? 'dark' : ''">
    <div v-if="$route.path !== '/'">
      <div
        class="dark-icon hidden-xs-only"
        v-if="themeConfig.darkMode.switch"
        @click="setDarkMode(!isDark)"
      >
        <span v-if="isDark" class="iconfont icon-yueliang"></span>
        <span v-else class="iconfont icon-taiyang"></span>
      </div>
    </div>
    <transition name="custom-classes-transition" enter-active-class="animated fadeIn">
      ...
      <div v-else>
        ...
        <BackTop />
        <Footer v-if="themeConfig.footer.show" />
      </div>
    </transition>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    const isDark =
      hour > this.$site.themeConfig.darkMode.on || hour < this.$site.themeConfig.darkMode.off

    const localDark = JSON.parse(localStorage.getItem("dark"))
    if (localDark !== null) {
      this.isDark = localDark
    } else {
